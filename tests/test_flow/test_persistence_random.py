...
class ModelFlowHarness:
    ...
    def query_and_check_entity(self, entity_name, expect_exact_call_count_matches=None):
        ...
        if self._cur_flow is None:
            self._cur_flow = self._builder.build()
            self._cur_model_memoized_values_by_entity_name = {}

        flow_exception = None
        try:
            flow_value = self._cur_flow.get(entity_name)
        ...
        context = ModelExecutionContext(
            ...
            versioning_mode=self._versioning_mode,
            memoized_values_by_entity_name=self._cur_model_memoized_values_by_entity_name,
            query_memoized_values_by_entity_name={},
    ...
    def _clear_cur_flow(self):
        self._cur_flow = None
        self._cur_model_memoized_values_by_entity_name = None
    ...
    def _compute_model_value(
        ...
        if entity_name in context.query_memoized_values_by_entity_name:
            return context.query_memoized_values_by_entity_name[entity_name]
        if entity_name in context.memoized_values_by_entity_name:
            return context.memoized_values_by_entity_name[entity_name]
        ...
        if binding.should_memoize:
            context.memoized_values_by_entity_name.update(values_by_entity_name)
        elif not binding.should_persist:
            context.query_memoized_values_by_entity_name.update(values_by_entity_name)
...
class ModelExecutionContext:
    ...
    memoized_values_by_entity_name = attr.ib()
    query_memoized_values_by_entity_name = attr.ib()
